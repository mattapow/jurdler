...
1) first word - a five letter word as the first guess start
2) solution word - a five letter target word. Can be "unknown"
...
# Examples
```
julia jurdler.jl raise trait y -1 10
```
...
function jurdler(ARGS)
    ...
    elseif length(ARGS) == 1
        solve(; first_guess=string(ARGS[1]))
    elseif length(ARGS) == 2
        first_guess = string(ARGS[1])
        solve(; first_guess=first_guess, solution=ARGS[2])
